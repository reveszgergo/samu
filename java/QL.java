...
public class QL {
	...
	public class Perceptron {
			...
			for (int i = 1; i < n_layers; ++i) {
				...
				for (int j = 0; j < n_units[i]; ++j) {
					...
					for (int k = 0; k < n_units[i - 1]; ++k) {
						weights[i - 1][j][k] =  -1 + (random.nextDouble() * 2);
					}
				}
			}
		}
		...
		public double operator(double image[]) {
			units[0] = image;
	...
	double max_ap_Q_sp_ap(double image[]) {
		...
		for (Iterator<Entry<SPOTriplet, Perceptron>> it = prcps.entrySet().iterator(); it.hasNext();) {
			...
			q_spap = ((Perceptron) thisEntry.getValue()).operator(image);
			;
	...
	SPOTriplet argmax_ap_f(String prg, double image[]) {
		...
		for (Iterator<Entry<SPOTriplet, Perceptron>> it = prcps.entrySet().iterator(); it.hasNext();) {
			Entry<SPOTriplet, Perceptron> thisEntry = (Entry<SPOTriplet, Perceptron>) it.next();
			double q_spap = ((Perceptron) thisEntry.getValue()).operator(image);
	...
	SPOTriplet operator(SPOTriplet triplet, String prg, double image[]) {
		...
		if (prev_reward > -java.lang.Double.MAX_VALUE && frqs.get(prev_action) != null) {
			...
			for (int z = 0; z < 10; ++z) {
				double nn_q_s_a = (prcps.get(prev_action)).operator(prev_image);
